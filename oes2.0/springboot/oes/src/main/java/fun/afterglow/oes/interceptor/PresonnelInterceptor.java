...
public class PresonnelInterceptor implements HandlerInterceptor {
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        HttpSession session = request.getSession();
        Presonnel presonnel = (Presonnel)session.getAttribute("presonnel");
        String url = request.getRequestURI();
        if (url.toLowerCase().indexOf("presonnel")>=0){
//            Presonnel presonnel = (Presonnel)session.getAttribute("presonnel");
            if (presonnel.getRoles().equals(Contant.ROLES_TEACHER)|presonnel.getRoles().equals(Contant.ROLES_ADMIN)){
                return true;
            }
